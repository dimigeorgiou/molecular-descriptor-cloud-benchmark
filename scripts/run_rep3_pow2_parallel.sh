#!/usr/bin/env bash
# Launch all rep3 pow2 Spot grids in parallel (one process per YAML).
set -euo pipefail
...
LOGDIR="$REPO/tmp/rep3_pow2_parallel"
mkdir -p "$LOGDIR"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] START parallel rep3 pow2 — ${#CONFIGS[@]} jobs" | tee -a "$MASTER"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  PIDS+=("$!")
  # tiny stagger so Batch submit / Sheets don't stampede
  sleep 8
done
...
# Wait for all; record exit codes
fail=0
for i in "${!PIDS[@]}"; do
  pid="${PIDS[$i]}"
  cfg="${CONFIGS[$i]}"
