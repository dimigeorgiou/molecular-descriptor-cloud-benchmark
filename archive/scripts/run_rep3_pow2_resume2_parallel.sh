#!/usr/bin/env bash
# Resume2: fill remaining MISSING/PARTIAL rep3 cells after AWS endpoint crashes.
set -euo pipefail
...
LOGDIR="$REPO/tmp/rep3_pow2_resume2"
mkdir -p "$LOGDIR"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] START resume2 parallel — ${#CONFIGS[@]} jobs" | tee -a "$MASTER"
